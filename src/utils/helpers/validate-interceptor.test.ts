...
import { IRequestInterceptor, IResponseInterceptor } from "../interfaces/interceptor-options.interface";
...
describe('Interceptor Validator', () => {
    it('should throw an error if both PATH and EXACT_PATH are in the by array', () => {
        ...
        expect(() => validateInterceptorOptions(config)).toThrow('Cannot have both PATH and EXACT_PATH in by array');
      });
      it('should throw an error if path is used without PATH or EXACT_PATH', () => {
        ...
        expect(() => validateInterceptorOptions(config)).toThrow('Path must be used with PATH or EXACT_PATH');
      });
      it('should throw an error if method is used without METHOD', () => {
        ...
        expect(() => validateInterceptorOptions(config)).toThrow('Method must be used with METHOD');
      });
      it('should throw an error if params are used without PATH', () => {
        ...
        expect(() => validateInterceptorOptions(config)).toThrow('Params must be used with PATH');
      });
      it('should throw an error if params are not rightly formatted in the path', () => {
        ...
        expect(() => validateInterceptorOptions(config)).toThrow('Params must be formatted like {param}');
      });
})