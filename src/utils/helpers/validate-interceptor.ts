...
export const validateInterceptorOptions = (
  ...
  const isByPathAndExactPath =
    options.by.includes(InterceptorByEnum.PATH) &&
    options.by.includes(InterceptorByEnum.EXACT_PATH);
  const hasPathButIsNotByPathOrExactPath =
    options.path &&
    !options.by.includes(InterceptorByEnum.PATH) &&
    !options.by.includes(InterceptorByEnum.EXACT_PATH);
  const hasMethodButIsNotByMethod =
    !!options.methods && !options.by.includes(InterceptorByEnum.METHOD);
  const hasParamsButIsNotByPath =
    !!options.params && !options.by.includes(InterceptorByEnum.PATH);
  const hasParamsButHasNoPath = !!options.params && !options.path;
  const isNotRightFormatted =
    options.params?.every((param) => options.path?.includes(`{${param}}`)) ===
    false;
